...
def execute_virtual_interfaces():
    ...
    script_path = '/home/bibin/msc_pro/docker_comp/interfaces2.sh'
...
def execute_get_namespace_ovs1():
    ...
    subprocess.run(veth_commd, check=True)
    veth_commd = ['sudo', 'ip', 'link', 'set', 'veth-r-inter', 'netns', namespace_path]
    subprocess.run(veth_commd, check=True)



def execute_get_namespace_ovs2():
    command = f"docker inspect ubun_cont_ovs2 | grep SandboxKey"
    result = subprocess.run(command, shell=True, capture_output=True, text=True)
    output = result.stdout.strip()
    start_index = output.find('"/') + 2
    end_index = output.find('"', start_index)
    ...
    namespace_path = '/'+ namespace_path
    print(namespace_path)
    veth_commd = ['sudo', 'ip', 'link', 'set', 'veth-b-inter', 'netns', namespace_path]
    subprocess.run(veth_commd, check=True)
...
def execute_ip_addr():
    ...
    command_ovs17 = f"docker exec ubun_cont_ovs1 ifconfig veth-r-inter up"
    command_ovs18 = f"docker exec ubun_cont_ovs1 ovs-vsctl add-port br0 veth-r-ovs"
    command_ovs19 = f"docker exec ubun_cont_ovs1 ovs-vsctl add-port br0 veth-r-inter"
    ...
    command_ovs23 = f"docker exec ubun_cont_ovs2 /usr/share/openvswitch/scripts/ovs-ctl start"
    command_ovs24 = f"docker exec ubun_cont_ovs2 ovs-vsctl add-br br0"
    command_ovs25 = f"docker exec ubun_cont_ovs2 ifconfig br0 up"
    command_ovs26 = f"docker exec ubun_cont_ovs2 ifconfig veth-b-inter up"
    command_ovs27 = f"docker exec ubun_cont_ovs2 ifconfig veth-b-ovs up"
    command_ovs28 = f"docker exec ubun_cont_ovs2 ovs-vsctl add-port br0 veth-b-inter"
    command_ovs29 = f"docker exec ubun_cont_ovs2 ovs-vsctl add-port br0 veth-b-ovs"
    result23 = subprocess.run(command_ovs23, shell=True, text=True)
    result24 = subprocess.run(command_ovs24, shell=True, text=True)
    result25 = subprocess.run(command_ovs25, shell=True, text=True)
    result26 = subprocess.run(command_ovs26, shell=True, text=True)
    result27 = subprocess.run(command_ovs27, shell=True, text=True)
    result28 = subprocess.run(command_ovs28, shell=True, text=True)
    result29 = subprocess.run(command_ovs29, shell=True, text=True)
